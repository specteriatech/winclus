...
(function () {
  ...
  var DICC = {
    en: {
      ...
      "Este sitio puede recibir estas cifras (solo números, sin nada personal) para saber cuántas personas consiguen hacer sus gestiones con Winclus. Se envían una vez por semana, solo si lo activas.": "This site can receive these figures (only numbers, nothing personal) to know how many people manage to complete their tasks with Winclus. They are sent once a week, only if you turn it on.",
      ...
      "Menos": "Less", "Más": "More", "Boca a la derecha": "Mouth to the right", "Boca a la izquierda": "Mouth to the left", "Fruncir los labios": "Pucker lips", "Sonreír": "Smile", "Meter el labio de abajo": "Tuck in lower lip",
  ...
  var SITUACIONES = [
    ["👁️", "Veo poco", "Letra grande y más contraste", "no veo bien"],
    ["👂", "No oigo bien", "Avisos en pantalla y subtítulos", "no oigo"],
    ["🖱️", "No puedo usar el ratón", "Mover el puntero con la cara", "no puedo usar el raton"],
    ["🔘", "Solo puedo pulsar un botón", "Winclus va marcando y tú pulsas", "pulsador"],
    ["🗣️", "No puedo hablar", "Dibujos y frases con voz", "no puedo hablar"],
    ["📖", "Me cuesta leer o entender", "La página en fácil y en voz alta", "no entiendo"],
    ["⌨️", "Me cuesta escribir", "Teclado en pantalla y dictado", "escribir"],
    ["🌀", "La pantalla me marea", "Sin destellos ni movimiento", "calma"]
  ];
  var situEl = el("div", { "class": "wcl-situ" });
  SITUACIONES.forEach(function (x) {
    var b = el("button", { "type": "button" });
    b.innerHTML = '<span class="ico" aria-hidden="true">' + x[0] + '</span><span>' + T(x[1]) + '</span><small>' + T(x[2]) + '</small>';
    b.addEventListener("click", function () { asistenteGuiado(x[3]); });
  ...
  if (opciones.metricas) {
    s.appendChild(filaSw("metricas_compartir", "Compartir mis cifras de uso con este sitio", enviarMetricasSiToca, "Este sitio puede recibir estas cifras (solo números, sin nada personal) para saber cuántas personas consiguen hacer sus gestiones con Winclus. Se envían una vez por semana, solo si lo activas."));
  }
