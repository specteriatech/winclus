...
(function () {
  ...
  var DICC = {
    en: {
      ...
      "Guardar mis ajustes en un archivo": "Save my settings to a file", "Cargar mis ajustes desde un archivo": "Load my settings from a file", "El archivo (.winclus) guarda ajustes, calibración de los ojos, frases y palabras aprendidas.": "The file (.winclus) keeps settings, eye calibration, phrases and learned words.",
      ...
      "Toca lo que te cuesta": "Tap what is hard for you", "Lo que más tocas del panel: ": "What you use most in the panel: ", "Boca a la derecha": "Mouth to the right", "Boca a la izquierda": "Mouth to the left", "Fruncir los labios": "Pucker lips", "Sonreír": "Smile", "Meter el labio de abajo": "Tuck in lower lip",
  ...
  var css = ''
    ...
    + '@keyframes wcl-late{0%,100%{box-shadow:0 0 0 0 rgba(47,79,216,0)}50%{box-shadow:0 0 0 6px rgba(47,79,216,.35)}}.wcl-situ.destacar button{animation:wcl-late 1s 3}html.wcl-calma .wcl-situ.destacar button,html.wcl-anim .wcl-situ.destacar button{animation:none;border-color:#2F4FD8}'
    // «Léemelo»: en la cabecera de cada sección
    + '.wcl-sec h2{display:flex;align-items:center;justify-content:space-between;gap:8px}.wcl-leeme{min-height:32px;padding:0 10px;border-radius:999px;border:1px solid #8892A6;background:#fff;color:#101F3D;font:600 12px "Segoe UI",system-ui,sans-serif;text-transform:none;letter-spacing:0;cursor:pointer}.wcl-leeme:hover{background:#E8F7F3}'
    + '.wcl-activo{margin:0;padding:0;list-style:none}.wcl-activo li{display:flex;align-items:center;gap:8px;min-height:32px;font-size:14px}.wcl-activo li::before{content:"";width:10px;height:10px;border-radius:50%;background:#0F7A70;flex:none}.wcl-activo li.nada::before{background:#8892A6}'
  ...
  // La gente no conoce los nombres de las funciones; conoce lo que le pasa. Cada botón
  // enciende lo adecuado por el asistente guiado y explica en una frase qué ha hecho.
  var s = seccion("¿Qué te cuesta?", "Toca lo que te pase y Winclus enciende lo que ayuda. Puedes tocar varios.");
  ...
  var SITUACIONES = [
    ["👁️", "Veo poco", "Letra grande y más contraste", "no veo bien", 6564],
    ["👂", "No oigo bien", "Avisos en pantalla y subtítulos", "no oigo", 6572],
    ["🖱️", "No puedo usar el ratón", "Mover el puntero con la cara", "no puedo usar el raton", 2546],
    ["🔘", "Solo puedo pulsar un botón", "Winclus va marcando y tú pulsas", "pulsador", 6195],
    ["🗣️", "No puedo hablar", "Dibujos y frases con voz", "no puedo hablar", 6517],
    ["📖", "Me cuesta leer o entender", "La página en fácil y en voz alta", "no entiendo", 7141],
    ["⌨️", "Me cuesta escribir", "Teclado en pantalla y dictado", "escribir", 2380],
    ["🌀", "La pantalla me marea", "Sin destellos ni movimiento", "calma", 35569]
  ];
  var situEl = el("div", { "class": "wcl-situ" });
  SITUACIONES.forEach(function (x) {
    var b = el("button", { "type": "button" });
    var ico = el("span", { "class": "ico", "aria-hidden": "true" }, x[0]);
    if (x[4]) { var img = el("img", { "alt": "", "width": "40", "height": "40", "loading": "lazy" }); img.src = PICTO_IMG + x[4] + "/" + x[4] + "_300.png"; img.onerror = function () { img.remove(); }; ico.textContent = ""; ico.appendChild(img); ico.dataset.emoji = x[0]; }
    b.appendChild(ico); b.appendChild(el("span", {}, x[1])); b.appendChild(el("small", {}, x[2]));
    b.addEventListener("click", function () { contarPanel(x[1]); asistenteGuiado(x[3]); });
    situEl.appendChild(b);
  });
  s.appendChild(situEl);
  var respuestaEl = el("div", { "class": "wcl-estado", "id": "wcl-respuesta", "aria-live": "polite" }, "");
  s.appendChild(respuestaEl);
  ...
  function activos() {
    ...
    if (a.cursor_grande) l.push("Cursor grande"); if (a.animaciones) l.push("Sin movimiento"); if (a.dalton !== "no") l.push("Colores corregidos"); if (a.calma) l.push("Modo calma");
    ...
    if (camaraActiva) l.push(a.modo_puntero === "ojos" ? "Puntero con los ojos" : "Puntero con la cabeza");
    if (a.barrido) l.push("Barrido con un pulsador"); if (tecVisible) l.push("Teclado en pantalla"); if (dictando) l.push("Dictado"); if (a.facil) l.push("Modo fácil");
    return l.map(T);
  }
  ...
  function apagarTodo() {
    ["contraste", "oscuro", "enlaces", "guia", "cursor_grande", "animaciones", "calma", "lupa_pantalla", "mascara", "dislexia", "sinimg", "lectura", "lector", "alertas_sonido", "subtitulos", "barrido", "facil"].forEach(function (k) { ajustes[k] = POR_DEFECTO[k]; });
    ajustes.texto = 100; ajustes.dalton = "no"; guardar();
  ...
  setInterval(function () { if (panel.classList.contains("abierto") && tabs.inicio.classList.contains("activa")) pintarActivo(); }, 1500);   // la cámara, el teclado o el dictado cambian sin pasar por «refrescos»
  tabs.inicio.appendChild(s);
  ...
  s.appendChild(filaSw("lector", "Lector de pantalla activado", activarLector, "Lee la página con voz y se maneja con el teclado: ↓ ↑ leen, h encabezados, l enlaces, b botones, f campos, Intro activa, F1 ayuda. Si ya usas NVDA, JAWS o VoiceOver, déjalo apagado: se pisarían."));
  tabs.oir.appendChild(s);
  ...
  s.appendChild(el("div", { "class": "wcl-ayuda" }, "Un tablero de dibujos (pictogramas ARASAAC): tocas los dibujos, se forma la frase y Winclus la dice con voz."));
  ...
  if (opciones.camara) {
    ...
    fOjosCalib.appendChild(botonGrande("Olvidar la calibración", "suave", function () { calibracion = null; escribirJSON("winclus.calibracion", null); reiniciarPuntero(); refrescos.forEach(function (f) { f(); }); }));
    ...
    fApr.agregar(botonGrande("Olvidar los clics", "suave", function () { clicsAprendidos = []; escribirJSON("winclus.clics", null); refrescarEstadoAprendizaje(); }));
    s.appendChild(fApr);
    tabs.cara.appendChild(s);
  } else {
  ...
  gQ.appendChild(fQ);
  s.appendChild(gQ);
  s.appendChild(el("div", { "class": "wcl-ayuda" }, "Con los ojos cerrados 1,2 s se abre el menú de clics: clic derecho, doble clic, arrastrar, rueda, teclado, leer, recentrar o pausar."));
  ...
  s.appendChild(el("div", { "class": "wcl-ayuda" }, "El archivo (.winclus) guarda ajustes, calibración de los ojos, frases y palabras aprendidas."));
  tabs.mas.appendChild(s);
  ...
  s.appendChild(botonGrande("Restablecer todo", "suave", function () {
    ...
    ["winclus.calibracion", "winclus.ojos_centro", "winclus.clics", "winclus.frases", "winclus.palabras", "winclus.consentimiento_camara", "winclus.visto"].forEach(function (k) { escribirJSON(k, null); });
  ...
  function asistenteGuiado(texto) {
    ...
    if (respuestaEl) respuestaEl.textContent = respuesta;   // en la pestaña Inicio queda escrito qué se ha hecho
