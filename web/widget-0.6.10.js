...
(function () {
  ...
  var POR_DEFECTO = {
    ...
    iman_activo: true, iman_radio_px: 90, iman_cabeza: false, calib_invisible: true, calib_modo: "normal", calib_lento: false, calib_punto_grande: false, calib_cabeza: true, puntero_externo: false, bordes_desplazan: true,
  ...
  var DICC = {
    en: {
      ...
      "Bajar y subir llevando el puntero al borde": "Scroll by moving the pointer to the edge", "Lleva el puntero al borde de abajo de la pantalla y la página baja sola; al borde de arriba, sube. Apártalo y para. También bajas abriendo la boca y subes con las cejas, o diciendo «baja» y «sube».": "Move the pointer to the bottom edge of the screen and the page scrolls down by itself; to the top edge, it scrolls up. Move it away and it stops. You can also scroll down by opening your mouth and up with your eyebrows, or by saying “down” and “up”.",
  ...
  function vuelta(tS) {
    ...
    tickClics(tS);
    if (!pausado) { tickBordes(tS); tickIman(tS); tickMenu(tS); tickTeclado(); avisarHover(); }
  ...
  var BORDE_PX = 56, bordeDesde = 0, bordeLado = 0, bordeAvisado = false;
  var bordeEl = el("div", { "class": "wcl-borde", "aria-hidden": "true" });
  function tickBordes(tS) {
    var H = window.innerHeight, lado = 0;
    if (ajustes.bordes_desplazan && !menuVisible && !tecVisible && !lupa && !calibrando && !arrastrando) {
      if (P.y >= H - BORDE_PX) lado = 1; else if (P.y <= BORDE_PX) lado = -1;
    }
    if (lado !== bordeLado) { bordeLado = lado; bordeDesde = tS; bordeEl.classList.remove("abajo", "arriba"); }
    if (!lado || tS - bordeDesde < 0.35) return;
    var prof = lado === 1 ? (P.y - (H - BORDE_PX)) / BORDE_PX : (BORDE_PX - P.y) / BORDE_PX;   // 0 al entrar en la franja, 1 pegado al borde
    var paso = lado * (3 + 9 * Math.min(1, Math.max(0, prof)));   // de 3 a 12 px por cuadro
    rueda(paso, false, P.x, Math.min(H - BORDE_PX - 20, Math.max(BORDE_PX + 20, P.y)));   // se desplaza lo que hay bajo el puntero, sin contar la franja del borde
  ...
  function pintarPictos() {
    ...
    if (pictoVista === "temas") {
      if (pictoCat === "Mis frases") pictoFrases.forEach(function (f) { var b = el("button", { "type": "button", "class": "picto frase-guardada" }, ""); b.appendChild(el("span", {}, f.texto)); b.addEventListener("click", function () { decirVoz(f.texto, true, true); avisar(f.texto); anotarHistorial(f.texto); }); rej.appendChild(b); });
    ...
    if (sugeridos.length) { sig.appendChild(el("span", { "class": "et" }, "Siguiente:")); sugeridos.slice(0, 6).forEach(function (n) { sig.appendChild(pictoBoton(pictoDatos(n), pictoElegir)); }); }
      ...
      if (rec.length) { sig.appendChild(el("span", { "class": "et" }, "Lo que más dices:")); rec.forEach(function (t) { var b = el("button", { "type": "button", "class": "picto reciente" }, ""); b.appendChild(el("span", {}, t)); b.addEventListener("click", function () { decirVoz(t, true, true); avisar(t); anotarHistorial(t); contar("pictos"); }); sig.appendChild(b); }); }
    }
  }
  ...
  function pictoDecir() { var t = pictoTexto(); if (t) { decirVoz(t, true, true); avisar(t); contar("pictos"); anotarHistorial(t); } else decirVoz("No hay frase todavía", true, true); }
  ...
  function activos() {
    ...
    if (a.barrido) l.push(a.barrido_modo === "pasos" ? "Barrido con dos pulsadores" : "Barrido con un pulsador"); if (tecVisible) l.push("Teclado en pantalla"); if (dictando) l.push("Dictado"); if (a.facil) l.push("Modo fácil");
    return l.map(T);
  }
  ...
  if (opciones.camara) {
    ...
    s.appendChild(filaSw("bordes_desplazan", "Bajar y subir llevando el puntero al borde", null, "Lleva el puntero al borde de abajo de la pantalla y la página baja sola; al borde de arriba, sube. Apártalo y para. También bajas abriendo la boca y subes con las cejas, o diciendo «baja» y «sube»."));
  ...
  s.appendChild(botonPeligroso("Restablecer todo", function () {
    ...
    ["winclus.calibracion", "winclus.ojos_centro", "winclus.clics", "winclus.frases", "winclus.palabras", "winclus.consentimiento_camara", "winclus.visto", "winclus.tableros", "winclus.pictos_historial"].forEach(function (k) { escribirJSON(k, null); });
  ...
  window.Winclus = {
    ...
    tickBordes: tickBordes,   // bajar y subir por los bordes (para pruebas)
