...
async function correr(page, nombre) {
  const r = await page.evaluate(async () => {
    ...
    return res.violations.map((v) => ({ id: v.id, impact: v.impact, help: v.help, nodos: v.nodes.map((n) => ({ sel: n.target.join(" "), msg: (n.any.concat(n.all, n.none)[0] || {}).message || "" })) }));
  });
  const n = r.reduce((s, v) => s + v.nodos.length, 0);
  console.log((n ? "MAL " : "OK  ") + nombre + ": " + r.length + " regla(s) incumplida(s), " + n + " elemento(s)");
  for (const v of r) {
...
(async () => {
  ...
  await page.evaluate(() => { const t = Array.from(document.querySelectorAll(".wcl-tec button")).find((b) => b.textContent === "Frases"); if (t) t.click(); });
  await page.waitForTimeout(150);
  ...
  await page.reload();
  await page.waitForFunction(() => window.Winclus);
  await page.evaluate(() => { const p = document.querySelector(".wcl-pausa"); p.style.display = "block"; Winclus.abrir(); });
  ...
  const manual = await page.evaluate(() => {
    ...
    const sw = document.querySelector(".wcl-sw"), swOn = document.querySelector('.wcl-sw[aria-checked="true"]') || sw;
    const pausa = document.querySelector(".wcl-pausa"), tecla = null;
    const casos = [
      ...
      ["borde de los botones − / +", cs(".wcl-mm button").borderTopColor, fondo(document.querySelector(".wcl-mm button").parentElement), 3],
      ["borde de las opciones", cs(".wcl-opc button").borderTopColor, fondo(document.querySelector(".wcl-opc button").parentElement), 3],
      ["foco del botón flotante (anillo)", "rgb(47, 79, 216)", "rgb(255, 255, 255)", 3],
      ["texto del aviso de estado", cs(".wcl-estado").color, fondo(document.querySelector(".wcl-estado")), 4.5],
    ];
  ...
  const teclado = await page.evaluate(() => {
    function lum(c) { const m = c.match(/\d+(\.\d+)?/g).map(Number); const [r, g, b] = m.slice(0, 3).map((v) => v / 255).map((v) => v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4)); return 0.2126 * r + 0.7152 * g + 0.0722 * b; }
    function ratio(a, b) { const x = lum(a), y = lum(b); return (Math.max(x, y) + 0.05) / (Math.min(x, y) + 0.05); }
    const t = getComputedStyle(document.querySelector(".wcl-tec .fila:not(.sug) button")), fondo = getComputedStyle(document.querySelector(".wcl-tec")).backgroundColor;
