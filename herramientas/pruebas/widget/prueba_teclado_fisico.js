...
(async () => {
  ...
  const nTab = await page.evaluate(() => document.querySelectorAll(".wcl-tec button[tabindex='-1']").length);
  ...
  while (pasos < 80 && !llegado) { await page.keyboard.press("Tab"); pasos++; llegado = await page.evaluate(() => !!document.activeElement.closest(".wcl-tec")); }
  comprobar(llegado, "con Tab se llega al teclado en pantalla", pasos + " pulsaciones");
  // hasta la tecla «q»
  let enQ = false; pasos = 0;
  while (pasos < 60 && !enQ) { enQ = await page.evaluate(() => document.activeElement.textContent === "q"); if (!enQ) { await page.keyboard.press("Tab"); pasos++; } }
  comprobar(enQ, "se llega a la tecla q");
  await page.keyboard.press("Enter");
  await page.waitForTimeout(100);
  const valor1 = await page.$eval("#campo", (e) => e.value);
  const focoTrasQ = await page.evaluate(() => document.activeElement.textContent);
  ...
  const iMayus = await page.evaluate(() => { const b = Array.from(document.querySelectorAll(".wcl-tec button")).find((x) => x.textContent === "Mayús"); b.focus(); return Array.from(document.querySelectorAll(".wcl-tec button")).indexOf(b); });
  await page.keyboard.press("Enter");
  await page.waitForTimeout(150);
  const trasMayus = await page.evaluate(() => ({ foco: document.activeElement.textContent, enTec: !!document.activeElement.closest(".wcl-tec"), q: !!Array.from(document.querySelectorAll(".wcl-tec button")).find((x) => x.textContent === "Q") }));
  ...
  // Frases
  await page.evaluate(() => { Array.from(document.querySelectorAll(".wcl-tec button")).find((x) => x.textContent === "Frases").focus(); });
  await page.keyboard.press("Enter");
  await page.waitForTimeout(150);
  const frases = await page.evaluate(() => Array.from(document.querySelectorAll(".wcl-tec button.frase")).map((b) => ({ t: b.textContent, tab: b.tabIndex })));
  comprobar(frases.length >= 1 && frases.every((f) => f.tab === 0), "las frases están en el teclado y son tabulables", frases.length + " frases");
  await page.evaluate(() => { window.__d = []; speechSynthesis.speak = (u) => { window.__d.push(u.text); }; Array.from(document.querySelectorAll(".wcl-tec button.frase"))[1].focus(); });
  await page.keyboard.press("Enter");
  await page.waitForTimeout(150);
  ...
  await page.evaluate(() => { const s = document.getElementById("wcl-lector"); if (s.getAttribute("aria-checked") !== "true") s.click(); });
  await page.waitForTimeout(200);
  ...
  await page.keyboard.press("ArrowDown");
  await page.waitForTimeout(100);
  const marcado = await page.evaluate(() => !!document.querySelector(".wcl-lector"));
