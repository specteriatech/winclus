...
const PRUEBAS = [
  ...
  ["prueba_arreglos.js", "Arreglos al vuelo para todas las ayudas técnicas: lang, alt a partir de lo que ya dice la página (nunca inventado), nombres de botones y enlaces con solo un icono, enlaces «aquí» con su contexto, etiquetas de campos, «Saltar al contenido», zona principal, foco visible, zoom permitido, títulos de iframes, cabeceras de tablas y controles en medios que arrancan solos; lo que no se puede arreglar sin riesgo se anota para quien mantiene el sitio; axe encuentra menos de la mitad de incumplimientos.", ["WCAG 1.1.1", "WCAG 1.3.1", "WCAG 1.4.4", "WCAG 2.4.1", "WCAG 2.4.4", "WCAG 2.4.7", "WCAG 3.1.1", "WCAG 3.3.2", "WCAG 4.1.2", "Res. 1519 CC1", "Res. 1519 CC10", "Res. 1519 CC17", "Res. 1519 CC24", "Res. 1519 CC26", "Res. 1519 CC27"]],
  ["prueba_monitor.js", "Winclus Monitor: vigilancia continua del sitio con historial, panel, informes con ACR y avisos por webhook cuando aparece una barrera nueva, una página deja de responder, el widget desaparece o se pierde la declaración; páginas del sitemap.", ["Res. 1519 Anexo 1", "WCAG 2.1 AA", "ITI VPAT 2.5", "EN 301 549"]],
  ["prueba_botones.js", "0.8.0: ocultar imágenes, saturación por grados, estructura de la página (títulos, zonas, enlaces), información al pasar, contraste inteligente (solo los textos que no llegan a 4,5:1, con la lista para el sitio), botón a la izquierda, panel grande y «Avisar de una barrera» por correo o POST.", ["WCAG 1.4.3", "WCAG 1.4.13", "WCAG 2.4.1", "WCAG 2.4.6", "WCAG 2.4.10", "EN 301 549 11.7", "EN 301 549 12.2"]],
  ["prueba_idiomas2.js", "El panel en portugués, francés, italiano y alemán: diccionarios completos (880 frases), traídos por el cargador según el idioma de la página, sin que se cuele español; data-ui manda sobre lang.", ["WCAG 3.1.1", "EN 301 549 11.7", "Res. 2893 Anexo 1 4.3.2 b"]],
  ["prueba_cargador.js", "Cargador ligero: widget.js pesa unos 3 KB y trae el widget minificado cuando la página ya se pintó, o antes si la persona ya lo usa; pasa data-* y nonce, Alt+Mayús+W lo pide, y cargarlo dos veces no duplica nada.", ["Rendimiento", "EN 301 549 11.7"]],
  ...
  ["prueba_traducir.js", "Traducción automática del contenido (0.8.2, data-traducir): «Otro idioma» ofrece «Traducir esta página» a 17 idiomas con el servicio de traducción del sitio (texto, alt, placeholder, title y aria-label, en lotes), cambia el lang de la página, no toca el panel de Winclus ni el código, «Volver al idioma original» restaura todo, y si el servicio falla la página queda como estaba; sin data-traducir no sale la opción ni se manda nada.", ["Res. 2893 de 2020 Anexo 1 4.3.2 b", "WCAG 3.1.1", "WCAG 3.1.2"]],
  ["prueba_clave.js", "Clave de sitio (0.8.2, data-clave): el cargador pide la configuración del panel a winclus.com/api/config y la pasa al widget (logo, nombre, pestañas ocultas), lo escrito en la línea manda, sin respuesta el panel sale igual, y al salir de la página el widget manda a /api/uso solo números (panel abierto y opciones tocadas) sin identificadores; sin clave no manda nada.", ["Ley 1581 de 2012", "WCAG 4.1.2"]],
  ["prueba_escanear.js", "Escáner en línea (winclus.com/escanear y /api/escanear): rechaza direcciones privadas o sin https, analiza una página con barreras en el Chromium del servidor (axe-core y comprobaciones de la Res. 1519), devuelve gravedad, criterio del Anexo 1 y puntaje, y la página pinta el resultado, lo descarga como informe y explica los errores.", ["Res. 1519 de 2020 Anexo 1", "WCAG 2.1 AA"]],
  ["prueba_panel.js", "Panel de cliente (winclus.com/panel): sin servidor configurado lo dice; con servidor pide el correo para el enlace de acceso; las pestañas del sitio siguen el patrón Tabs; los límites por plan coinciden con el esquema SQL; axe sin incumplimientos.", ["WCAG 2.4.3", "WCAG 4.1.2"]],
  ["prueba_personalizar.js", "Panel con la marca del sitio (0.8.2): data-logo y data-nombre en la cabecera («… con Winclus» traducido), data-ocultar quita pestañas (botón oculto, flechas del teclado las saltan, «¿Qué te cuesta?» no lleva a ellas) e Inicio y Más salen siempre; sin atributos el panel es el de siempre.", ["WCAG 2.4.3", "WCAG 4.1.2", "EN 301 549 11.5"]],
  ["prueba_declaracion.js", "Generador de declaración de accesibilidad (winclus.com/declaracion): la vista previa cambia al escribir, cita la Ley 1618, la Res. 1519 y la 2893 para entidades públicas (y NTC 5854 y WCAG para privadas), lista las excepciones, no descarga sin los datos mínimos, el archivo descargado es HTML válido con lang=es y lo escrito vuelve al recargar.", ["Res. 1519 de 2020 Anexo 1", "Res. 2893 de 2020", "WCAG 3.1.1"]],
  ["prueba_precios.js", "Página de precios (winclus.com/precios): cuatro planes con precio y lo que incluye, el plan gratis con el panel completo, tabla frente a UserWay con la cifra de pruebas al día, preguntas plegables accesibles y sin cobro por visitas en ningún plan.", ["Res. 2893 de 2020", "WCAG 1.3.1"]],
  ["prueba_idiomas_demo.js", "Página «El panel en 18 idiomas» (winclus.com/idiomas): un selector con cada idioma en su propia lengua; al elegir uno el panel se abre solo en ese idioma (japonés, árabe volteado, inglés, portugués), el elegido queda marcado y la línea de instalación lo lleva; un código inexistente cae a español.", ["WCAG 3.1.1", "WCAG 3.1.2", "EN 301 549 11.7", "Res. 2893 Anexo 1 4.3.2 b"]],
...
const json = { fecha: fecha.toISOString(), commit: commit(), version: (fs.readFileSync(path.join(RAIZ, "web/winclus-widget.js"), "utf8").match(/var VERSION = "([^"]+)"/) || [])[1], pruebas: resultados.length, comprobaciones: resultados.reduce((s, r) => s + r.ok + r.mal, 0), fallan, resultados };
