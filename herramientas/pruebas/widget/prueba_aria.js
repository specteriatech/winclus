...
(async () => {
  ...
  const despues = await page.evaluate(() => ({ foco: (Winclus.caja.activeElement || document.activeElement).id, sel: (Winclus.caja.activeElement || document.activeElement).getAttribute("aria-selected"), visible: getComputedStyle(Winclus.caja.getElementById((Winclus.caja.activeElement || document.activeElement).getAttribute("aria-controls"))).display !== "none" }));
  ...
  // --- botones − / + ---
  const mm = await page.evaluate(() => { const b = Winclus.caja.querySelectorAll("#wcl-panel-ver .wcl-mm button"); const d = Winclus.caja.getElementById(b[0].getAttribute("aria-describedby")); return { menos: b[0].getAttribute("aria-label"), mas: b[1].getAttribute("aria-label"), valor: d && d.textContent }; });
  ...
  // --- voz ---
  const voz = await page.evaluate(() => { const s = Winclus.caja.getElementById("wcl-voz-nombre"); return s && s.getAttribute("aria-live"); });
  ...
  const vivo = await page.evaluate(() => { const v = Winclus.caja.querySelector(".wcl-vivo"); const r = v.getBoundingClientRect(); return { texto: v.textContent, live: v.getAttribute("aria-live"), role: v.getAttribute("role"), oculto: r.width <= 1 && r.height <= 1, ariaHidden: v.getAttribute("aria-hidden") }; });
  ...
  const repite = await page.evaluate(() => { const e = Winclus.caja.getElementById("wcl-estado"); let n = 0; const o = new MutationObserver(() => n++); o.observe(e, { childList: true, characterData: true, subtree: true }); return new Promise((res) => { setTimeout(() => { o.disconnect(); res(n); }, 100); }); });
  comprobar(repite === 0, "la región de estado no se toca si el texto no cambia");

  // --- calibración ---
  const calib = await page.evaluate(() => { const c = Winclus.caja.querySelector(".wcl-calib"); return { role: c.getAttribute("role"), modal: c.getAttribute("aria-modal"), label: c.getAttribute("aria-label") }; });
