...
- `node prueba_idiomas2.js`: con el servidor local, páginas en pt, fr, it y de: el cargador trae `idiomas/xx.json` antes del widget, el panel sale traducido y sin español; los cuatro archivos cubren las 880 claves de `DICC.en`. Para añadir un idioma: copiar `web/idiomas/pt.json`, traducir y guardar como `xx.json`.
